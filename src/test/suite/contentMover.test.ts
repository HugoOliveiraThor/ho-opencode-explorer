...
suite('ContentMover', () => {
  ...
  test('moves a skill directory from global to local', () => {
    ...
    const from = path.join(globalSkills, 'alpha', 'SKILL.md');
    const dest = mover.moveSkill(from, 'global', ws);
    assert.strictEqual(dest, path.join(ws, localSkills, 'alpha'));
    assert.ok(fs.existsSync(path.join(ws, localSkills, 'alpha', 'SKILL.md')));
    assert.ok(!fs.existsSync(path.join(globalSkills, 'alpha')));
  });

  test('moves a skill from local to global', () => {
    fs.mkdirSync(path.join(ws, localSkills, 'beta', 'SKILL.md'), { recursive: true });
    const from = path.join(ws, localSkills, 'beta', 'SKILL.md');
  ...
  test('moves a command file', () => {
    fs.mkdirSync(globalCommands, { recursive: true });
    fs.writeFileSync(
    ...
    assert.strictEqual(dest, path.join(ws, localCommands, 'gamma.md'));
    assert.ok(fs.existsSync(path.join(ws, localCommands, 'gamma.md')));
  });

  test('rejects when target already exists', () => {
    fs.mkdirSync(path.join(globalSkills, 'dup'), { recursive: true });
    fs.mkdirSync(path.join(ws, localSkills, 'dup'), { recursive: true });
    assert.throws(() => mover.moveSkill(path.join(globalSkills, 'dup', 'SKILL.md'), 'global', ws));
  });

  test('throws when local target has no workspace', () => {
    assert.throws(() =>
      mover.moveSkill(path.join(globalSkills, 'alpha', 'SKILL.md'), 'global', undefined),
    );
  });
});
